...
const navVariantClasses =
  "bg-transparent px-0 whitespace-nowrap text-[16px] leading-[19px] font-bold text-white transition-colors hover:text-white/85";

const activeNavVariantClasses =
...
export function NavItem({ label, href, active = false }: NavItemProps) {
  return (
    <li className="relative">
      <Link href={href} className={cn(active ? activeNavVariantClasses : navVariantClasses)}>
        {label}
      </Link>
